...
当前执行计划（SSOT）：`docs/plan/2026-03-03-technical-debt-remediation-v0.10.3.md`
...
| Testing | Rust + vitest + JNI + Android 单元测试（实时数量以 CI 输出为准） |
...
cargo test -p tingyuxuan-jni                            # JNI 测试

# 代码质量红线（强制门禁）
npm run quality:redline                 # 红线检查（仅生产代码）
```
...
红线检查范围仅包含生产代码目录：`src`、`src-tauri/src`、`crates/*/src`、`android/app/src/main`。
Compose 声明式 UI 函数不纳入函数级红线统计（由界面组件拆分规范单独约束）。
...
- **DDD/SSOT 执行闸门（必遵守）**：
  ...
  2. 每次里程碑推进必须同步更新当前执行计划（`docs/plan/2026-03-03-technical-debt-remediation-v0.10.3.md`）
...
| `get_api_key` | `service: String`（仅 `llm`） | `Option<String>` | 获取 API Key |
| `save_api_key` | `service, key`（仅 `llm`） | `()` | 保存 API Key |
