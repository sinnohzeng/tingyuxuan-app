...
[Tauri Command] ──────────────────────────────────────────┐
  ...
  3. emit RecorderStarting（微加载过渡）                  │
  4. 获取活动窗口名（上下文感知提示词）                    │
  5. 创建历史记录（status: recording）                    │
  6. 启动录音器 Actor（发送 Start 命令）                   │
  7. emit RecordingStarted 事件                          │
        │                                                │
        ▼                                                │
[Recorder Actor: 专用 OS 线程]                            │
  ...
  - 前端在 4 分钟显示 60 秒倒计时；5 分钟自动触发 stop     │
        │                                                │
        ▼ (用户按完成/再按快捷键)                          │
...
<PipelineEvent>              │                             │
                             ...
                             │  RecorderStarting → show    │  更新 appStore 状态
...
| 窗口 | 标签 | 尺寸 | 可见性 |
|------|------|------|--------|
| FloatingBar | `floating-bar` | 220x56 (AI 助手扩展到 420x360) | 按需 show/hide |
| Settings | `settings` | 640x520 | 按需 show |

FloatingBar 可见性由事件桥接控制：
- `RecorderStarting` / `RecordingStarted` → show + focus
- `Error` → show + focus
